...
public class DroolsEngineImpl implements DroolsEngine {
    ...
    /**
     * Variable with some temp employee name for usage.
     */
    private String[] employeeNames = {"Tom", "Adam", "Evan", "Adit", "Glen",
	    "Sara", "Arjun", "Raja", "Max", "Krish"};
    /**
     * During for a single day.
     */
    public static final int DAY_DURATION = 86400000;
    /**
     * Limiter for substring.
     */
    public static final int LIMITER = 10;

    /*
     * This method is the implementation for drools rules.
     * @see com.goks.fortune.drools.DroolsEngine#executeDrools
     * (int, int, boolean, java.lang.String)*/
    @Override
    public final DailySchedule[] executeDrools(final int totalEmployees, final
	    int totalDays, final boolean random,
	    final String rulesFile) throws DroolsParserException, IOException {

	KieServices kieServices = KieServices.Factory.get();
	Resource resource = kieServices.getResources()
		.newFileSystemResource(rulesFile);
	...
	for (int i = 0; i < supportDays.length; i++) {
	    ...
	    supportDays[i]
		    .setToday(new Date(new Date().getTime() + DAY_DURATION
		+ (i * DAY_DURATION)).toString().substring(0, LIMITER));
	    supportDays[i].setYesterday(new Date(new Date().getTime()
		    + (i * DAY_DURATION)).toString().substring(0, LIMITER));
	...
	for (int i = 0; i < supportDays.length; i++) {
		...
		supportDays[i].setFirstShiftEmpId(employees[numberArray[j]]
			.getId());
	    ...
	    employees[supportDays[i].getFirstShiftEmpId()]
		    .setPreviousShiftDate(supportDays[i].getToday());
	    employees[supportDays[i].getFirstShiftEmpId()]
		    .setTotalShifts(employees[supportDays[i]
			    .getFirstShiftEmpId()].getTotalShifts() + 1);
	    j = 0;
	    supportDays[i].setComplete(false);
	    while (!supportDays[i].isComplete()) {
		KieSession kSession = kContainer.newKieSession();
		supportDays[i].setLastShiftEmpId(employees[numberArray[j]]
			.getId());
		supportDays[i].setComplete(true);
		kSession.insert(employees[numberArray[j]]);
		kSession.insert(supportDays[i]);
	    ...
	    employees[supportDays[i].getLastShiftEmpId()]
		    .setTotalShifts(employees[supportDays[i]
			    .getLastShiftEmpId()].getTotalShifts() + 1);
	}
    ...
    /**
     * @param max max limit for the array.
     * @param random true if random array is needed.
